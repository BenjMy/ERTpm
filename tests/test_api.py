...
file_selection = ['HOTBENT2_6X1020211101_1330.Data']
...
if do_invert:
    ...
    table_to_invert = select_table(
        table, which='all', col_check='invert', col_needed='finv'
    )
    print('data sets to invert are:\n{}'.format(table_to_invert))
    if table_to_invert.empty:
        ...
        for i, r in table_to_invert.iterrows():
            ...
            finv = r['finv']
            fref = None
            fyield = invert(fName=finv, mesh='mesh.bms', lam=50, err=0.05, opt=True)
            fvtk = next(fyield)
