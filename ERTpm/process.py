...
class ERTdataset():
    ...
    data_headers = ['meas', 'a', 'b', 'm', 'n',
                    'r', 'k', 'rhoa', 'ip', 'v', 'curr', 'ctc', 'stk', 'datetime',
    ...
    def to_ubc(self, fname, w_rhoa, w_ip, w_err, data_cols, elec_cols, rounding=6, sep=" "):
        ...
        with open(fname, 'a') as file_handle:
            for ab, g in inj_groups:
                ...
                for i, r in g.iterrows():
                    ...
                    file_handle.write(sep)
                    file_handle.write("{:8.8f}".format(r['r']))
                    file_handle.write("\n")
                file_handle.write("\n")
    ...
    def plot_together(self, fname, plot_columns, valid_column='valid', dir_proc='.'):
        ...
        for col in plot_columns:
            ...
            plt.savefig(fig_dirfname, dpi=180)
            plt.close()
...
def __process__(f, args):
    ...
    if args.odf == 'bert':
        ...
        ds.to_ubc(fdat, args.w_rhoa, args.w_ip, args.w_err, data_cols, elec_cols)
    else:
        raise ValueError('invalid output data format')
    print('output data file: ', fdat)
    # report
    report_columns = ['rec_valid', 'k_valid', 'rhoa_valid', 'v_valid', 'ctc_valid', 'stk_valid', 'valid']
